...
import { add_dot, convert_price_from_db } from "./modules/utils/priceFuncs.js"
import { getDecimalPlaces } from "./modules/consts/decimalPlaces.js"
import { filterBreadcrumbers, formatBreadcrumbers } from "./modules/utils/breadcrumberFinder.js"
...
window.shopSetup = async () => {
    ...
    if(!shopInfos){
        let arr = await get_shopInfos_from_db(connection, userInfo.id)
        shopInfos = arr[0]
        websiteIdLookup = arr[1]
    }

    if(!products) products = await get_all_products_from_db(connection, userInfo.id)

    let productsValues = getValuesFromObj(products)
    let productLength = productsValues.length

    productNodes = await get_products_component(productsValues)
...
async function get_products_component(products){
    let nodes = {}
    
    for(let i=products.length-1; i>=0; i--){
        let product = products[i]
        const shopInfo = shopInfos[product.website_id];
        product.price = await get_most_recent_price(connection, product.id)
        let imgSrc = get_img_src(product.id)
        let faviconSrc = get_favicon_src(product.website_id, shopInfo.faviconExtension)
        let websiteName = shopInfo.name
...
async function update_invalid_prices(connection, products, productValues=null){
    ...
    for(let i=newPrices.length-1; i>=0; i--){
        ...
        let id = product.id
        await save_price(connection, currency, value, id, date, false)
...
async function getShopInfo(mainUrl){
    let websiteId = websiteIdLookup[mainUrl]
    
    if(!websiteId){
        let shopInfo = await get_shopWebsiteInfo(connection, null, mainUrl)
        if(shopInfo){
...
window.handleStore = async () => {
    ...
    if(!product.alreadyStored) product[ProductProperties.BREADCRUMB] = formatBreadcrumbers(product[ProductProperties.BREADCRUMB])
    ...
    try{
        if(storeInfo.productInfo){
            ...
            if(product.alreadyStored){
                await store_user_shop_product(connection, userInfo.id, product.id, product.update_in)
                addProductToProducts(product, get_img_src(product.id), get_favicon_src(shopInfo.id, shopInfo.faviconExtension), shopInfo.name)
            }else{
                let [newProduct, _] = await store(connection, userInfo.id, product, null, storeInfo.shopInfo)
                addProductToProducts(newProduct, product[ProductProperties.IMG][ImgProperties.TAG].src, get_favicon_src(shopInfo.id, shopInfo.faviconExtension), shopInfo.name)
            }
        }
...
async function fetchProduct(url){
    if(url){
        try{
            ...
            let shopInfo = await getShopInfo(mainUrl)
            
            if(shopInfo){
                let product = await get_product_by_url(database, url, shopInfo.id)

                if(product){
                    product.price = await get_most_recent_price(database, product.id)
                ...
                if(product){
                    setupProductDisplay(product, get_favicon_src(shopInfo.id, shopInfo.faviconExtension), shopInfo.name)
