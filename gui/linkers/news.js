...
import {
  gather_info_from_browser,
  filter_knownWebsites,
  get_newsWebsiteInfos_by_score,
  get_prepared_articles_from_websites,
  get_newsWebsiteInfo_from_url,
  get_all_categories_from_website,
  create_newsWebsiteInfos,
} from './modules/news/newsFacade.js';
import {
  store_news_websites,
...
} from './modules/news/storageNews.js';
...
import {
  ...
  cleanAppend,
  createElement,
} from './modules/utils/htmlUtils.js';
import {
  getDisplayElement,
  displayKeywordTags,
  displayKeywordArticles,
} from './modules/news/newsDisplay.js';
import PagesSelector from './components/PagesSelector.js';
import {
  get_word_set_from_article_codes,
  verify_articles_code_obj_for_keys,
} from './modules/news/articleKeyFinder.js';
...
import { chooseForMe, recommendMe } from './modules/news/recommendation.js';

let unknownGatheredInfo,
  knownGatheredInfo,
  keywords,
  articles,
  wordSetObj,
  ...
  recommendedPages,
  newsInfos,
  recommendedNewsInfos,
  recommendedArticles,
  scanningHistory,
  recommendedArticlesHasChanged,
  newsTime,
  discoverTime,
  addingWebsite;
let articleHasChanged,
  keywordsLength = 0,
  numKeywordArticles = 0,
  alreadyShowedKeywords = false,
  currentSection = 'articles';

const sectionFunctions = {
  articles: () => displayArticles(),
  keywords: () => displayKeywords(),
  discover: () => displayDiscover(),
};
...
window.newsSetup = async () => {
  ...
  articleHasChanged =
    addingWebsite =
    scanningHistory =
    alreadyShowedKeywords =
    recommendedArticlesHasChanged =
      false;

  if (!newsInfos)
    newsInfos = await get_newsWebsiteInfos_by_score(database, userInfo.id);
...
window.update_news = async () => {
  ...
  if (Object.keys(newsInfos).length > 0) {
    ...
    element.append(
      StatusMessage(
        null,
        createElement('div', 'article-spinner'),
        'Getting articles...',
        null
      )
    );
...
window.refresh = () => {
  if (currentSection === 'articles') update_news();
  else if (currentSection === 'discover') update_recommendations();
};
...
async function update_recommendations() {
  discoverTime = new Date();

  let element = document.getElementById('discover-bottom');
  clearChildren(element);

  if (Object.keys(recommendedNewsInfos).length > 0) {
    clearChildren(element);
    element.append(
      StatusMessage(
        null,
    ...
    recommendedArticles = await get_prepared_articles_from_websites(
      recommendedNewsInfos
    );

    clearChildren(element);
    recommendedArticlesHasChanged = true;
    ...
    element.append(
      StatusMessage(
        ...
        undefined,
        'Hmm... bad news!',
        "I couldn't discover any new news website for you :(. Sorry!"
      )
    );
  }
}
...
function displayUnknownWebsites(unknownWebsites, counter = undefined) {
  displayGatheredWebsites(
    Object.values(unknownWebsites),
    UnknownNewsWebsite,
    'unknown-websites',
    counter
  );
...
function displayGatheredWebsites(
  infos,
  component,
  elementID,
  counter = undefined
...
window.handle_addToNews = async () => {
  // let newsInfos = get_valids_unknown_websitesInfo(unknownGatheredInfo)
  await store_news_websites(
    database,
    get_valids_unknown_websitesInfo(unknownGatheredInfo),
    newsInfos,
    userInfo.id
  );
  // let selected = get_valids_known_websitesInfo(knownGatheredInfo)
  // console.log(selected)
  await store_categories_after_gathering_of_known(
    database,
    get_valids_known_websitesInfo(knownGatheredInfo)
  );
...
window.changeSlider = (toRight = true) => {
  ...
  if (toRight) {
    document.getElementById('right-btn').style.webkitAnimationName =
      'out-slider';
    ...
    document.getElementById('right-btn').style.webkitAnimationName =
      'on-slider';
    document.getElementById('left-btn').style.webkitAnimationName =
      'out-slider';
...
window.handleAddKeyword = async () => {
  ...
  if (text) {
    ...
    if (isKeywordValid(text)) {
      if (!(text in keywords)) {
        let id = await store_keyword(database, text, userInfo.id);
        ...
        let selected = verify_articles_code_obj_for_keys(articles, wordSetObj, [
          text,
        ]);
...
async function displayKeywords() {
  if (!keywords) {
    ...
    keywords = await retrieve_keywords(database, userInfo.id);
  }
  ...
  if (!alreadyShowedKeywords) {
    if (keywordsLength > 0) {
      ...
      let keywordsObj = verify_articles_code_obj_for_keys(
        articles,
        wordSetObj,
        keys
      );
...
function displayArticles(numElementsPerPage = 30) {
  updateTimer('last-update-hour', newsTime);

  if (articleHasChanged) {
    pages = genericDisplayArticles(
      'articles-top',
      'articles-bottom',
      articles,
      numElementsPerPage
...
function displayRecommendedArticles(numElementsPerPage = 30) {
  if (recommendedArticlesHasChanged) {
    updateTimer('last-update-hour', discoverTime);

    recommendedPages = genericDisplayArticles(
      'discover-top',
      'discover-bottom',
      recommendedArticles,
      numElementsPerPage,
      (id) => store_user_news_website(database, userInfo.id, id)
    );
    recommendedArticlesHasChanged = false;
  }
}

function genericDisplayArticles(
  topElementId,
  bottomElementId,
  articles,
  ...
  clearChildren(topElement);
  clearChildren(bottomElement);

  if (Object.keys(articles).length > 0) {
    const pages = getDisplayElement(
      articles,
    ...
    if (pages.length > 1) {
      topElement.prepend(
        new PagesSelector(pages.length, (index) => {
          let articlesElements = bottomElement;
    ...
    bottomElement.append(pages[0]);

    return pages;
  } else
    document.bottomElement.append(
      StatusMessage(
        undefined,
        undefined,
        'No Articles',
...
async function displayDiscover(numElementsPerPage = 30) {
  updateTimer('last-update-hour', discoverTime);

  if (!recommendedNewsInfos) {
    recommendedArticlesHasChanged = true;

    const recommendations = await recommendMe(
      Object.values(newsInfos),
      database
    );
    recommendedNewsInfos = await create_newsWebsiteInfos(
      database,
      chooseForMe(recommendations, 10)
    );

    await update_recommendations();
  } else {
    displayRecommendedArticles(numElementsPerPage);
  }
}

window.changeSectionBar = (e) => {
  const { id, offsetLeft, offsetWidth } = e.target;
  const section = id.split('-')[0];

  let sectionBar = document.getElementById('section-bar');
  sectionBar.style.transform = `translateX(${offsetLeft}px)`;
  sectionBar.style.width = `${offsetWidth}px`;

  document
    .getElementById(`${currentSection}-btn`)
    .classList.remove('above-bar');
  e.target.classList.add('above-bar');

  hideElement(currentSection);
  showElement(section);

  sectionFunctions[section]();
...
window.scanHistory = async () => {
  if (!scanningHistory) {
    ...
    let infos = await gather_info_from_browser(
      database,
      databaseInfo,
      newsInfos,
      7
    ...
    if (
      Object.keys(unknownGatheredInfo).length > 0 ||
      Object.keys(knownGatheredInfo).length > 0
    ) {
  ...
  if (keywordsLength === 0)
    ...
    showStatusMessage(
      ...
      StatusMessage(
        ...
        undefined,
        'No Articles',
        `I couldn't find any articles with ${
          keywordsLength > 1 ? 'these keywords' : 'this keyword'
        }.<br>Try <em>adding some more above</em>!`
      )
    );
}
...
async function getManageWebsites() {
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (!newsInfo) {
      newsInfo = await retrieve_newsWebsiteInfo(database, undefined, url);
      newsInfos[url] = newsInfo;
    }

    fragment.append(
      NewsWebsiteElement(newsInfo, deleteNewsWebsite, deleteNewsCategory)
    );
  }
...
window.handleManageWebsites = async () => {
  cleanAppend(
    document.getElementById('all-websites'),
    await getManageWebsites()
  );
...
async function addNewsWebsite(url, successMessage = true) {
  try {
    ...
    if (newsInfo) {
      newsInfos[url] = newsInfo;
      await store_news_website(database, newsInfo);
      console.log(newsInfo);
      ...
      if (successMessage)
        showMessage(
          ...
          `${newsInfos[url].name} was saved in your news websites!`
        );
      ...
      showMessage(
        MESSAGE_TYPES.ERROR,
        'Annn... what?!',
        `Wow, am I too dumb or this is not a news website? Check the url and try again`
      );
  } catch (error) {
    console.log(error);
    showMessage(
...
window.handleAddNewsWebsite = async () => {
  ...
  if (newsInfo) {
    ...
    if (length > 0) {
      ...
      let [validCategories, invalidCategories] =
        await get_all_categories_from_website(categoriesUrls, newsInfo);
      ...
      if (length > 0) {
        categories = [];
        for (let i = 0; i < length; i++)
          categories.push(validCategories[keys[i]].name);
        ...
        showMessage(
          ...
          `${
            isAlreadyInNews
              ? ''
              : `${newsInfo.name} was saved in your news websites!<br>`
          }The following ${
            length > 1 ? 'categories were' : 'category was'
          } added: ${categories.join(', ')}`
        );
      } else
        showMessage(
          MESSAGE_TYPES.SUCCESS,
          ...
          `${
            isAlreadyInNews
              ? ''
              : `${newsInfo.name} was saved in your news websites!<br>`
          }None of the categories were valid, so none were added!`
        );
    } else {
      if (isAlreadyInNews)
        showMessage(
          MESSAGE_TYPES.ERROR,
          'Duplicate Website',
          `${newsInfo.name} is already in your news websites!`
        );
      else
        showMessage(
          MESSAGE_TYPES.SUCCESS,
          'Another One in the Basket',
          `${newsInfo.name} was saved in your news websites!`
        );
    }
