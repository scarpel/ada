import {
  ...
  LAST_USER,
  USERS,
} from '../linkers/modules/consts/databaseConsts.js';
...
window.addEventListener('DOMContentLoaded', async () => {
  let last_user = await getLastUser();
  ...
  LAST_UPDATE_DATES = await getLastUpdateDate();
...
async function getLastUser() {
  const { id } = (await database.get(`SELECT * FROM ${LAST_USER}`)) || {};
  return id
    ? await database.get(
        `SELECT id, name, gender, birthday FROM ${USERS} WHERE id = ${id}`
      )
    : undefined;
}
...
function loadPage(
  url,
  maskColor = 'black',
  maskStartOrientation = 'right-oriented',
  maskEndOrientation = 'left-oriented'
) {
...
window.showMessage = (messageType, title, text, callback = () => {}) => {
  let okBtn = document.getElementById('ok-btn'),
    icon;
  okBtn.onclick = () => {
    _hideMessage(icon, () => callback(true));
  };
  ...
  if (messageType === MESSAGE_TYPES.QUESTION) {
    ...
    noBtn.onclick = () => {
      _hideMessage(icon, () => callback(false));
    };
    okBtn.innerText = 'hmm, ok!';
  } else {
    icon = document.getElementById(
      messageType === MESSAGE_TYPES.SUCCESS ? 'success-icon' : 'error-icon'
    );
...
async function getLastUpdateDate() {
  let updates = await database.all(`SELECT * FROM ${LAST_UPDATE_DATE}`);
  let obj = {};
...
async function createLastUpdateDate() {
  let obj = {};
  ...
  for (let i = keys.length - 1; i >= 0; i--) {
    ...
    await database.run(
      `INSERT INTO ${LAST_UPDATE_DATE} VALUES(${id},${key.toLowerCase()},0)`
    );
    obj[id] = 0;
  }
...
window.updateLastUpdateDate = async (id, timestamp) => {
  try {
    await database.run(
      `UPDATE ${LAST_UPDATE_DATE} SET date=${timestamp} WHERE id=${id}`
    );
...
window.getNewsDecisionTree = () => {
  if (!newsDecisionTree) {
    newsDecisionTree = build_decision_tree_from_str(
      read(NEWS_DECISION_TREE).toString()
    );
  }
