...
export default function ArticleXX1X(
  node,
  id = undefined,
  onClickFunction = () => {},
  addFunction = null
  ...
  let base = createElement(
    'div',
    `article-xx1x ${node[ArticleItems.CODE]}`,
    id
  );
  let func = (event) => {
    onClickFunction(node[ArticleItems.LINK], event);
    base.classList.add('read');
  };
  ...
  if (node[ArticleItems.IMG]) {
    ...
    imgLink.append(
      createImg('article-xx1x-img', node[ArticleItems.IMG].content)
    );
    meta.append(imgLink);
  } else base.classList.add('no-img');

  if (addFunction) {
    const addBtn = createElement('div', 'add-btn');
    addBtn.innerText = '+ Add';
    addBtn.onclick = () => {
      addFunction(newsInfo.id);
    };
    base.prepend(addBtn);
  }

  let faviconA = createSimpleCustomA((event) => {
    onClickFunction(newsInfo.url, event);
  });
  ...
  if (node[ArticleItems.HEADER]) {
    ...
    if (headerNode.link) {
      header = createSimpleCustomA((event) => {
        onClickFunction(headerNode.link, event);
      });
