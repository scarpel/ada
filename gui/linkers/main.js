...
import {
  build_decision_tree,
  build_decision_tree_from_str,
} from './modules/utils/decisionTree.js';
import {
  getArticleCounterArray,
  getTypesAndSubtypes,
  getWebsitesId,
} from './modules/utils/newsUtils.js';
...
import { get_newsWebsiteInfos_by_score } from './modules/news/newsFacade.js';
import { get_websites_by_types } from './modules/news/storageNews.js';
import { chooseForMe, recommendMe } from './modules/news/recommendation.js';

window.mainSetup = async () => {
  changeTitleBarBackground('rgb(241, 241, 241)');
  changeContentBackground('rgb(241, 241, 241)');

  setGreetingMessage();
  // const users = await getXMoreFrequent(100);
  // await getNewsWebsitesFromUsers(users);
  // scoreUsersWebsites(users, [1]);
  // console.log(getXWithBestScore(users, 5));

  const websites = Object.values(
    await get_newsWebsiteInfos_by_score(database, 12)
  );
  const web = await recommendMe(websites);
  console.log(web);
  console.log(chooseForMe(web, 5));

  // console.log(getScores(groupEntries(getValues(10000, 10, 10)), [1, 2, 4]));
...
function getArticlesArray() {
  let data = [];
  let text = read(
    'C:\\Users\\Guilherme\\Desktop\\test_ada\\articles.txt'
  ).toString();
  text = text.split(',');
  for (let i = 0; i < text.length; i++) {
    data.push(
      getArticleCounterArray(
        getHTMLCounter(text[i].replace(/(\r\n|\n|\r)/gm, ''))
      )
    );
  }
  text = read(
    'C:\\Users\\Guilherme\\Desktop\\test_ada\\naoArticles.txt'
  ).toString();
  text = text.split(',');
  for (let i = 0; i < text.length; i++) {
    data.push(
...
function setGreetingMessage() {
  ...
  document.getElementById(
    'greeting'
  ).innerHTML = `${hourGreeting}, <b>${userInfo.name}</b>!`;
}
...
window.signOut = async () => {
  await database.run(`DELETE FROM ${LAST_USER}`);
  userInfo = {};
  goBack();
};
