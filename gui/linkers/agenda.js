...
const todoRE = /^todo ./;
let controller, lastSliderPosition, atTodo = false;

window.agendaSetup = async () => {
    ...
    displayTodos(document.getElementById("all-todos"), ...sort_todos(await retrieve_todos(database, userInfo.id)), handleTodoBlur, handleCheckboxChange, addNewToDo, removeTodo)
}

async function handleTodoBlur(event){
    ...
    if(!element.innerText) removeTodo(parent)
        ...
        if(id === ""){
            parent.id = await store_todo(database, userInfo.id, element.innerText)
...
async function addToDo(title=undefined, onBlurFunc = handleTodoBlur, onChecked = handleCheckboxChange, removeFunc= removeTodo, delay=0.1){
    let item = await store_todo(database, userInfo.id, title, undefined, undefined, true)
...
function addOperation(agendaEvent){
    if(agendaEvent.title){
        if(atTodo) addToDo(agendaEvent.title)
        else{
...
async function removeOperation(agendaEvent){
    let matches = await select_events_that_match(database, agendaEvent)
...
async function addNewEvent(agendaEvent){
    agendaEvent["id"] = await store_event(database, agendaEvent, userInfo.id, true)
...
window.saveEvent = () => {
    ...
    if(title && start_at){
        ...
        addNewEvent(AgendaEvent(title, start_at, end_at, priority))
        hideScheduleEvent()
