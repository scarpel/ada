...
window.handleNewAccount = async () => {
  ...
  if (emailRE.test(email)) {
    if (!(await hasEmail(email))) {
      ...
      if (
        password &&
        password === document.getElementById('same-password').value
      ) {
        try {
          password = await window.argon2.hash(password, {
...
window.saveNewUser = async () => {
  ...
  if (name) {
    ...
    if (birthday.isSameOrBefore(moment(), 'day')) {
      ...
      let id = await addNewUser(email, password, name, gender, birthday);
      if (id) goBack();
...
async function hasEmail(email) {
  return database.get(`SELECT email FROM ${USERS} WHERE email=?`, [email]);
}

export async function addNewUser(email, password, name, gender, birthday) {
  await database.run(`INSERT INTO ${USERS} VALUES(?,?,?,?,?,?,?)`, [
    null,
    email,
    password,
    name,
  ...
  return await get_last_id(database, USERS);
}
