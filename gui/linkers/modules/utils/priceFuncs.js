...
function add_dot(currency, price){
    ...
    if(decimalPlaces){
        ...
        if(!dots.has(price[len-decimalPlaces-1])) return price.replace(dotsRegex, "");
        else{
            const dotInBetween = len-decimalPlaces;
            return `${price.slice(0, dotInBetween).replace(dotsRegex, "")}.${price.slice(dotInBetween)}`
        }
    }
...
function clean_up_price(array){
    return array.filter(value => isNumber.test(value)).join(".")
}
...
export { isDigit, get_first_price_index, get_currency, get_price_and_currency, check_after_price, 
    ...
    convert_price_to_db, convert_prices_from_db, clean_up_price }
