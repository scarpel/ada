...
class Counter {
  ...
  remove = (name, number = 1) => {
    if (name in this.values) {
      this.values[name] -= number;
      if (this.values[name] <= 0) delete this.values[name];
    }
  ...
  getSortedArray = () =>
    Object.entries(this.values).sort(([, a], [, b]) => b - a);
}
...
class FrequencyCounter extends Counter {
  ...
  updateMostFrequentKey = (lastUpdatedKey) => {
    if (
      ...
      get(this.values, lastUpdatedKey, 0) >
        get(this.values, this.mostFrequentKey, 0)
    )
      this.mostFrequentKey = lastUpdatedKey;
  };
...
class ArticleTagCounter extends Counter {
  ...
  verify = (decisionTree, acceptancePercentage = 0.5, print = false) => {
    const d = decisionTree.evaluate(this.values);
    const v = d >= acceptancePercentage;
    if (print) console.log(d, v);
    return v;
  };
...
class InterpreterEnvironment {
  constructor(
    minTagsForAcceptance = 3,
    minNumOccurences = 5,
    maxTagsForAcceptance = 10,
    decisionTree = getNewsDecisionTree(),
    acceptancePercentage = 0.8
  ) {
    this.tagCounter = new ArticleTagCounter();
    this.articleClasses = new Counter();
    this.confirmedArticleClasses = new Counter();
  ...
  mergeArticleClasses = () => {
    this.articleClasses.union(this.confirmedArticleClasses);
  };

  createNewsWebsiteInfo = (url, maxFailedAttemps = 1) => {
    return new NewsWebsiteInfo(
      undefined,
      url,
      undefined,
      new Set(Object.keys(this.articleClasses.values)),
      undefined,
      maxFailedAttemps
    );
  };
...
class NewsWebsiteInfo extends WebsiteInfo {
  constructor(
    ...
    url,
    name = '',
    types = [WEBSITE_TYPES.GENERAL],
    score = 0,
    articlesInfo = {},
    lastTagsUpdate = moment(),
    maxFailedAttemps = 1
  ) {
    super(id, url, '', name);
  ...
  static fromObject = (obj, url = undefined) => {
    return new NewsWebsiteInfo(
      obj.id,
      url ? url : treat_all_url(obj.url),
      obj.name,
      [obj.type, obj.subtype],
      obj.score,
      undefined,
      moment.unix(obj.last_update)
    );
  };
}
...
function createArticleElementsTable(
  img = null,
  header = null,
  title = null,
  description = null,
