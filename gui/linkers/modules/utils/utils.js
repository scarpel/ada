...
function calculate_article_code(articleElements) {
  return [
    get_article_code(articleElements[ArticleItems.IMG]),
    get_article_code(articleElements[ArticleItems.HEADER]),
    get_article_code(articleElements[ArticleItems.TITLE]),
    get_article_code(articleElements[ArticleItems.DESCRIPTION]),
  ].join('');
}
...
function isArticleElementsTableEmpty(articleTable) {
  return (
    !articleTable[ArticleItems.TITLE] &&
    !articleTable[ArticleItems.IMG] &&
    !articleTable[ArticleItems.HEADER] &&
    !articleTable[ArticleItems.DESCRIPTION]
  );
}
...
function sumArticleElementsTableCode(articleTable) {
  ...
  let code = get(
    articleTable,
    ArticleItems.CODE,
    calculate_article_code(articleTable)
  );
...
function binary_search(
  value,
  array = [],
  comparissonFunction = (a, b) => a < b,
  equalFunction = (a, b) => a === b
) {
  let start = 0;
...
function binary_search_for_insertion(
  value,
  array = [],
  comparissonFunction = (a, b) => a < b
) {
...
function sequentialArray(length, start = 0) {
  const arr = new Array[length]();
  for (let i = 0, num = start; i < length; i++, num++) arr[i] = num;
  return arr;
}
...
function getRandoms(min, max, num, int = true) {
  const interval = max - min;
  const getValue = int ? (value) => ~~value : (value) => value;

  if (interval > num) {
    const randomNums = [];

    while (randomNums.length < num) {
      const randInt = getValue(random(min, max));
      if (!randomNums.includes(randInt)) randomNums.push(randInt);
    }

    return randomNums;
  } else return sequentialArray(num, min);
}
...
export {
  ...
  lazySplit,
  removeNone,
  sortArticleNodesByClass,
  gimmeRandom,
  calculate_article_code,
  isArticleElementsTableEmpty,
  getArticlesLink,
  index,
  get,
  count,
  lTrim,
  rTrim,
  getValuesFromObjAsArray,
  getValuesFromObj,
  hasSomething,
  sumArticleElementsTableCode,
  getArrayWithN,
  get_favicon_src,
  binary_search_for_insertion,
  binary_search,
  sumArticleCode,
  zeroArray,
  getUnique,
