...
function get_website_name(reference, title, html) {
  let site_name = html.find('property="og:site_name');
  let start = null;
  let end = null;
  ...
  if (site_name !== -1) {
    ...
    if (prop) return prop.split('|')[0].split('-')[0].trim();
  }
  ...
  if (end) {
    if (!start) start = 0;
    return titleArray
      .slice(start ? start : 0, end)
      .map((value) => capitalize(value))
      .join(' ');
  } else return `${capitalize(reference[0])}${reference.slice(1)}`;
}

function get_news_website_type(html, get_secondary_type = true) {
  ...
  if (description) {
    ...
    if (keysLength > 1 && keysLength < 4) {
      if (
        types.values[WEBSITE_TYPES.TECH] &&
        types.values[WEBSITE_TYPES.SCIENCE] &&
        (types.values[WEBSITE_TYPES.ENTERTAINMENT] ||
          types.values[WEBSITE_TYPES.CULTURE])
      )
        types.add(WEBSITE_TYPES.TECH, types.size * 2);
      if (
        keysLength === 2 &&
        types.values[WEBSITE_TYPES.ENTERTAINMENT] === 1 &&
        !types.values[WEBSITE_TYPES.LIFESTYLE]
      )
        types.pop(WEBSITE_TYPES.ENTERTAINMENT);
    }

    console.log(types);
    keysLength = types.keys().length;

    if (keysLength < 5) {
      if (keysLength > 1) {
        let sorted = types.getSortedArray();
        if (sorted[0][1] / types.size > 0.5) {
          return get_secondary_type
            ? [sorted[0][0], sorted[1][0]]
            : [sorted[0][0]];
        } else if (get_secondary_type)
          return [WEBSITE_TYPES.GENERAL, sorted[0][0]];
      } else if (keysLength === 1) return [types.keys()[0]];
    }
  }
...
function get_website_description(html) {
  const header = html.slice(0, html.indexOf('</head>'));

  for (let i = 0, length = descriptionNames.length; i < length; i++) {
    let description = get_property_meta_tag(
      header,
      descriptionNames[i],
      'content'
    );
...
function isLinkValid(src) {
  if (!src) return false;
  else return src.indexOf('http') === 0 || src[0] === '/';
}
...
function get_sub_name(mainUrl, urlArray = null) {
  if (!urlArray) {
    ...
    return `${urlArray[first]}${urlArray
      .slice(first + 1, last)
      .map((value) => capitalize(value))
      .join('')}`;
  } else {
...
function get_basic_website_info(mainUrl, title, html, getType = false) {
  ...
  if (getType) obj['types'] = get_news_website_type(html);
...
function format_url(url = '', websiteUrl) {
  return url[0] === '/' ? `${websiteUrl}${url}` : url;
}
...
export {
  ...
  isLinkValid,
  get_sub_name,
  get_basic_website_info,
  add_protocol,
  get_domain,
  treat_url,
  protocolsRE,
  treat_all_url,
  get_news_website_type,
