...
async function download_img(url, savePath, fileName, fileExtensionWithoutDot){
        ...
        return await Axios({
            ...
            .then(response => {
                if(response.status === 200){
                    try{
                        response.data.pipe(window.parent.createWriteStream(window.parent.path_join(savePath, `${fileName}.${fileExtensionWithoutDot}`), {flags: "w+"}))
                        return true
                    }catch (err){
                        throw err
                        return false
