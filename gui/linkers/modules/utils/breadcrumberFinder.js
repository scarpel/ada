import { decode_html_text, isEndTag } from "./htmlUtils.js"
            ...
            if(spaceIndex != -1){
                ...
                if(breadcrumberElements.has(element)){
                    ...
                    if(divEnd>index){
                        ...
                        if(!breadClass.includes("related")) return [breadClass, divEnd+1]
                    }
                }
            }
...
function filterBreadcrumbers(breadcrumbers, websiteName=""){
    if(breadcrumbers && breadcrumbers.length){
        const first = breadcrumbers[0].toLowerCase();
        console.log(first, websiteName)

        if(first === websiteName.toLowerCase() || ['go', 'go back', 'voltar'].some(value => first.includes(value))){
            breadcrumbers.shift();
        };
        console.log(breadcrumbers)

        return breadcrumbers.filter(breadcrumber => breadcrumber.replace(invalidChars, "").length);
    }else return breadcrumbers;
}

function formatBreadcrumbers(breadcrumbers){
    if(breadcrumbers){
        const separatedBreadcrumbers = breadcrumbers.map(value => decode_html_text(value.toLowerCase()).split(separatorsRegex));
        const wordSet = new Set(separatedBreadcrumbers.slice(1).flat());
    
        return {
            mainCategory: separatedBreadcrumbers[0].join(","),
            additionalCategories: [...wordSet].join(",")
        }
    }else return {}
}

export {getBreadcrumbers, findUnknownBreadcrumberStart, findBreadcrumberStart, filterBreadcrumbers, formatBreadcrumbers}