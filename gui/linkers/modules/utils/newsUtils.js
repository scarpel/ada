...
import { get, getUnique } from './utils.js';

function get_news_website_basic_info(html, url) {
  return get_basic_website_info(
    url,
    get_tag_content(html, 'title', 0, html.length),
    html,
    true
  );
}

const titleCorrespondingTags = [
  'header',
  'text',
  'container',
  'image',
...
function getArticleCounterArray(counter, isTrue = true) {
  return [
    get(counter.values, 0, 0),
    get(counter.values, 1, 0),
    get(counter.values, 2, 0),
    get(counter.values, 3, 0),
    get(counter.values, 4, 0),
    get(counter.values, 5, 0),
    isTrue,
  ];
}
...
function format_authors_name(authorsName, start = 'By', connector = 'and') {
  ...
  if (length === 1) return `${start} ${authorsName[0]}`;
    ...
    if (length === 2)
      return `${start} ${authorsName[0]} ${connector} ${authorsName[1]}`;
    else {
      let last = length - 1;
      return `${start} ${authorsName.slice(0, last).join(', ')} ${connector} ${
        authorsName[last]
      }`;
    }
  }
...
function get_category_name(category, title) {
  ...
  if (start !== undefined) {
    ...
    return `/${
      category[length - 1] === '/' ? category.slice(0, length - 1) : category
    }`;
  }
}
...
function verifyCategoryEnd(category) {
  ...
  return category[lastPosition] === ':'
    ? category.slice(0, lastPosition)
    : category;
}
...
function getTypesAndSubtypes(values) {
  const arr = [];
  console.log(values);
  ...
  values.forEach((element) => {
    if (element.type !== null) {
      ...
      if (element.subtype !== null) arr.push(element.subtype);
    }
  });
