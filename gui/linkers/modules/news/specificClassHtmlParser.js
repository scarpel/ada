import {
  ...
  SimpleTagNode,
  SimpleUnaryTagNode,
} from '../utils/htmlTreeStructure.js';
import {
  index_after,
  get_until,
  get_until_if_exists,
} from '../utils/strFuncs.js';
...
import { format_url, isLinkValid } from '../utils/websiteUtils.js';
import { ArticleItems, LinkedList, WebsiteInfo } from '../utils/dataTypes.js';
import { ArticleItem } from '../utils/newsDataType.js';
import { article_groups } from './newsFacade.js';

const defaultDesirableTags = new Set([
  'div',
  'p',
  'time',
  'a',
...
const getTags = new Set(['div', 'li', 'tr', 'section', 'article']);
...
class SpecificClassHTMLParser {
  ...
  prepare_html = (rawHtml) => {
    return get_body_or_main(rawHtml)
      .replaceAll('\n', '')
      .replaceAll('\t', ' ')
      .replaceAll('\r', '')
  ...
  _parseTags = (html, classes) => {
    ...
    while (this.index < this.endHtml) {
      ...
      if (!getDiv) {
        if (getTags.has(tag)) {
          let tagClass = get_until_if_exists(
            html,
            '"',
            index_after(html, 'class="', startIndex, endIndex),
            endIndex
          );
          ...
          this.aLinks.push(
            get_until_if_exists(
              html,
              '"',
              index_after(html, 'href="', startIndex, endIndex),
              endIndex
            )
          );
        ...
        if (isEndTag(tag)) {
          ...
          if (
            ...
            this.hasText[this.hasText.length - 1]
          ) {
            this.parents[parentsLength - 1].content = html
              .slice(this.index, startIndex)
              .trim();
          }
          ...
          if (parentsLength === 1 && this.parents[0].filterChildren()) {
          ...
          if (this.desirableTags.has(tag)) {
            ...
            if (unaryTags.has(tag)) {
              if (tag === 'img')
                this.parents[this.parents.length - 1].addChild(
                  this._get_img(html, startIndex, endIndex)
                );
            } else {
              this.hasText.push(true);
              let node;
      ...
      if (!getDiv) {
        if (getTags.has(tag)) {
          ...
          if (tagClass in articlesInfo) {
            ...
            if (!invertedArticleElementsDict[tagClass]) {
              ...
              for (let i = 0; i < length; i++) {
                ...
                if (element && element.tag)
                  obj[element.tagClass] = [element.tag, articleElementCode];
              }
            ...
            currentInvertedArticleElements =
              invertedArticleElementsDict[tagClass];
        ...
        if (isEndTag(tag)) {
          ...
          if (
            ...
            this.parents[parentsLength - 1].content = decode_html_text(
              html.slice(this.index, startIndex)
            );
          ...
          if (parentsLength === 1) {
            getDiv = false;
            currentArticleElements[ArticleItems.CODE] = calculate_article_code(
              currentArticleElements
            );
            if (currentArticleElements[ArticleItems.CODE] === '0000')
              console.log(currentArticleElements);
            currentArticleElements[ArticleItems.NEWS_INFO] = newsInfo;

            if (!currentArticleElements[ArticleItems.LINK])
              currentArticleElements[ArticleItems.LINK] = this.parents[0].link;

            if (byClass)
              classNodes[this.parents[0].tagClass].push(currentArticleElements);
            else {
              let newCode =
                article_groups[currentArticleElements[ArticleItems.CODE]];
              let obj = classNodes[newCode];
              if (!obj)
                classNodes[newCode] = new LinkedList(currentArticleElements);
              else obj.push(currentArticleElements);
            }
          } else {
            let node = this.parents[parentsLength - 1];

            let elementArray = get(
              currentInvertedArticleElements,
              node.tagClass,
              null
            );
            if (elementArray && elementArray[0] === node.tag) {
              if (elementArray[1] === ArticleItems.IMG) {
                node.compactImg();
                node.content =
              ...
              if (elementArray[1] === ArticleItems.AUTHOR) {
                if (!currentArticleElements[ArticleItems.AUTHOR])
                  currentArticleElements[ArticleItems.AUTHOR] = [];
                currentArticleElements[ArticleItems.AUTHOR].push(
                  new ArticleItem(node)
                );
              } else
                currentArticleElements[elementArray[1]] = new ArticleItem(node);
            }
          }
          ...
          if (this.desirableTags.has(tag)) {
            ...
            if (unaryTags.has(tag)) {
              if (tag === 'img') {
                ...
                if (elementArray && elementArray[0] === 'img')
                  currentArticleElements[elementArray[1]] = new ArticleItem(
                    img
                  );
              ...
              if (html[endIndex - 1] !== '/') {
                ...
                if (getTags.has(tag)) node = new SimpleDivNode(tagClass);
                  ...
                  if (tag === 'a') {
                    ...
                    if (node.link) {
                      node.link = format_url(node.link, newsInfo.url);
                      if (!(ArticleItems.LINK in currentArticleElements))
                        currentArticleElements[ArticleItems.LINK] = node.link;
                      aLink = node.link;
                    }
                  }
                }
  ...
  _get_img = (html, startIndex, endIndex) => {
    ...
    let src = get_until_if_exists(
      html,
      '"',
      index_after(html, 'src="', startIndex, endIndex),
      endIndex
    );
    this.index = endIndex + 1;

    if (!isLinkValid(src)) {
      let noscriptStart = html.find(
        '<noscript',
        endIndex,
        endIndex + this.noscriptOffset
      );
      if (noscriptStart !== -1) {
        ...
        if (imgStart !== -1) {
          ...
          src = get_until_if_exists(
            html,
            '"',
            index_after(html, 'src="', imgStart, imgEnd),
            imgEnd
          );
        }
  ...
  _add_img_from_picture = (html) => {
    ...
    if (imgStart !== -1) {
      ...
      this.parents[this.parents.length - 1].addChild(
        this._get_img(html, imgStart, imgEnd)
      );
    }
  };
  ...
  _parseNonUsefulTag = (html, tag) => {
    ...
    if (endTag !== -1) {
      if (tag !== 'script') {
        let numPossibleTags = this._numAllPossibleTags(
          html,
          tag,
          this.index,
          endTag
  ...
  _numAllPossibleTags = (html, tag, start, end) => {
    ...
    while (indexPossibleTag !== -1 && indexPossibleTag < end) {
      numPossibleTags += 1;
      indexPossibleTag = html.find(
        `<${tag}`,
        index_after(html, '>', indexPossibleTag, end),
        end
      );
    }
