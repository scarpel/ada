...
async function get_prepared_articles_from_websites(
  newsNewsiteInfos,
  specificParser = new SpecificClassHTMLParser()
) {
  let articles = await get_articles_from_websites(
    newsNewsiteInfos,
    specificParser
  );
...
async function get_articles_from_websites(
  newsNewsiteInfos,
  specificParser = new SpecificClassHTMLParser()
) {
  ...
  for (let i = 0; i < length; i++) {
    console.log(validWebsites[i]);
    articles.push(
      specificParser.parseArticleElements(htmls[i], validWebsites[i])
    );
  }
...
function get_x_most_frequent_categories(tagsCounter, numberTags) {
  return tagsCounter
    .keys()
    .sort((a, b) => (tagsCounter.values[a] > tagsCounter.values[b] ? -1 : 1))
    .slice(0, numberTags);
}
...
function filter_sorted_urls(sortedUrls, newsWebSites, minLengthSet = 2) {
  ...
  for (let i = keys.length - 1; i >= 0; i--) {
    ...
    if (key.slice(0, 5) === 'https') {
      ...
      else if (sortedUrls[key].size >= minLengthSet)
        unknownWebsites[key] = sortedUrls[key];
    }
  }
...
async function get_websites_from_browser_in_interval(
  newsWebsitesInfos,
  browserInfo,
  ...
  return filter_sorted_urls(
    sort_by_url(
      await get_urls_from_interval(startDate, currentDate, browserInfo)
    ),
    newsWebsitesInfos
  );
}

async function get_unknown_newsWebsitesInfo(
  unknownWebsites,
  parser = new ArticleHTMLParser(),
  interpreter = new ArticleHTMLInterpreter(),
  specificParser = new SpecificClassHTMLParser(),
  predictor = new ArticlePredictor()
) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    let newsInfo = await retrieve_newsWebsiteInfo_from_url(database, url);
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (html) {
      let articleClasses = interpreter
        .interpret(parser.parse(html))
        .getArticleClasses();
      if (articleClasses.size > 0) {
        ...
        if (articleElementsLength > 0) {
          ...
          let newsInfo = new NewsWebsiteInfo(
            null,
            basicInfo.mainUrl,
            basicInfo.websiteName,
            basicInfo.type,
            length,
            articleElements
          );
...
async function get_all_categories_from_websites(sortedUrls, newsWebsitesInfo) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    let [categories, categoriesUrl] = get_possible_categories_and_urls(
      websiteInfo,
      [...sortedUrls[websiteUrl]]
    );

    if (categories.length > 0) {
      categoriesInfo.push([websiteInfo, categories]);
      allCategoriesUrl.push(...categoriesUrl);
    } else if (!websiteInfo.id || websiteInfo.alreadyStored)
      unknownWebsites[websiteInfo.url] = [{}, new Set(), websiteInfo];
  }
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (categoriesAdded > 0) {
      if (newsInfo.id && !newsInfo.alreadyStored)
        knownWebsites[newsInfo.url] = [
          categoriesObj,
          invalidCategories,
          newsInfo,
        ];
      else
        unknownWebsites[newsInfo.url] = [
          categoriesObj,
          invalidCategories,
          newsInfo,
        ];
    } else if (!newsInfo.id || newsInfo.alreadyStored)
      unknownWebsites[newsInfo.url] = [
        categoriesObj,
        invalidCategories,
        newsInfo,
...
async function get_all_categories_from_website(
  categoriesUrls,
  newsWebsiteInfo
) {
  ...
  if (length > 0) {
    ...
    for (let i = 0; i < length; i++)
      categories.push(categoriesUrls[i].slice(mainUrlLength));
...
async function gather_info_from_browser(
  connection,
  browserInfo,
  newsWebsitesInfo,
  daysBefore = 7,
  ...
  let [knownUrls, unknownUrls] = await get_websites_from_browser_in_interval(
    newsWebsitesInfo,
    browserInfo,
    undefined,
    daysBefore
  );
  let unknownInfos = await get_unknown_newsWebsitesInfo(
    unknownUrls,
    parser,
    interpreter,
    specificParser,
  ...
  await update_newsWebsiteInfos(
    connection,
    newsWebsitesInfo,
    Object.keys(knownUrls)
  );

  let categories = await get_all_categories_from_websites(
    { ...unknownUrls, ...knownUrls },
    { ...unknownInfos, ...newsWebsitesInfo }
  );
  return categories;
}

async function get_newsWebsiteInfo_from_url(
  url,
  html = null,
  parser = new ArticleHTMLParser(),
  interpreter = new ArticleHTMLInterpreter(),
  ...
  if (articleClasses.size) {
    let articleInfos = predictor.predictFromClasses(
      specificParser.parse(html, articleClasses),
      randomFactor
    );
...
function sort_articles_by_group(
  articles,
  websiteArticlesGroup = get_article_group_obj()
) {
...
async function get_newsWebsiteInfos_by_score(
  connection,
  userId,
  onlyNFirsts = undefined
) {
  let result = await connection.all(
    `SELECT n.* FROM ${NEWS_WEBSITES} n, ${USERS_NEWS_WEBSITES} u WHERE u.user_id=? AND u.website_id = n.id ORDER BY score DESC`,
    [userId]
  );
  ...
  if (resultLength > 0) {
    let newsInfos = await create_newsWebsiteInfos(connection, result);

    for (let i = onlyNFirsts; i < resultLength; i++)
      newsInfos[result[i].url] = undefined;

    return newsInfos;
  } else return {};
}

async function create_newsWebsiteInfos(connection, websites) {
  let newsInfos = {};

  for (let i = 0, length = websites.length; i < length; i++) {
    let newsInfo = NewsWebsiteInfo.fromObject(websites[i]);
    await fill_up_newsWebsiteInfo(connection, newsInfo);
...
async function update_newsWebsiteInfos(connection, newsWebsitesInfo, urls) {
  for (let i = urls.length - 1; i >= 0; i--) {
    let url = urls[i];
    let newsInfo = newsWebsitesInfo[url];

    if (!newsInfo) {
      newsWebsitesInfo[url] = await retrieve_newsWebsiteInfo(
        connection,
        null,
        url
      );
...
async function setManageWebsites(urls, length) {
  let fragment = document.createDocumentFragment();

  for (let i = 0; i < length; i++) {
    let newsInfo = newsInfos[urls[i]];
    if (!newsInfo) {
      newsInfo = await retrieve_newsWebsiteInfo(database, undefined, urls[i]);
...
export {
  get_articles_from_websites,
  get_newsWebsiteInfo_from_url,
  article_groups,
  sort_articles_by_group,
  sort_all_articles_by_group,
  get_websites_from_browser_in_interval,
  get_unknown_newsWebsitesInfo,
  get_all_categories_from_websites,
  get_newsWebsiteInfos_by_score,
  update_newsWebsiteInfos,
