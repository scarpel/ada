  ...
  flatParents = () => {
    ...
    if (length > 1) {
      ...
      for (let i = length - 1; i > 0; i--) {
        const tag = this.parents[i];

        if (containerTags.has(tag.tag)) {
          if (tags.length > 0) {
      ...
      this.parents[0].addChildren([...containers, ...tags]);
    }
  };
  ...
  _numAllPossibleTags = (tag, start, end) => {
    ...
    while (indexPossibleTag !== -1 && indexPossibleTag < end) {
      if (this._elegiblePossibleTag(indexPossibleTag)) {
        if (allowedTagFollowUps.has(this.html[indexPossibleTag + tagLength]))
          numPossibleTags += 1;
        else break;
      }

      indexPossibleTag = this.html.find(
        tag,
        index_after(this.html, '>', indexPossibleTag, end),
        end
      );
    }
    ...
    while (this.index > -1 && this.index < this.endHtml && counter < this.max) {
      ...
      if (isEndTag(tag)) {
        ...
        if (this.desirableTags.has(tagWithoutSlash)) {
          if (
            containerTags.has(tagWithoutSlash) &&
            this.hasText[this.hasText.length - 1] &&
            this.index + 1 !== startTag
          ) {
            this.tags.push('text');
          }
        ...
        if (this.desirableTags.has(tag)) {
          ...
          if (containerTags.has(tag)) {
            let tagClass = get_until_if_exists(
              this.html,
              '"',
              index_after(this.html, 'class="', startTag, endTag),
              endTag
            );
            ...
            this.tags.push(tag);
            if (this.html[endTag - 1] === '/' && ambiguosTags.has(tag))
              this.tags.push(`/${tag}`);
          }
        } else {
          tag = tag.trim();
  ...
  _parseTags = () => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (isEndTag(tag)) {
        ...
        const tagNode = this.parents[parentsLength - 1];
        tagNode.tag = tag.slice(1);

        if (parentsLength > 1) {
          this.parents[parentsLength - 2].addChild(tagNode);
        } else {
          return;
        }
        ...
        if (unaryTags.has(tag))
          this.parents[this.parents.length - 1].addChild(new UnaryTagNode(tag));
        else {
