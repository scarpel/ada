...
function getDisplayElement(
  ...
  if ('xx1x' in preparedArticles) {
    ...
    if (length1x10) {
      infos['xx1x']['numElements'] = Math.ceil(
        articles.length / (length1x10 / infos['1x10']['numElements'])
      );
    }
  }
  ...
  while (length > 1) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      let articles = displayFunctions[articleCode](
        array,
        info,
        onClickFunction,
        info['numElements'],
...
function displayXX1X(
  ...
  for (; index < numElements; index++) {
    ...
    if (node) {
      base.append(ArticleXX1X(node, undefined, onclickFunction, addFunction));
    } else {
      isFull = false;
      break;
    }
...
function display1X10(
  ...
  for (; index < numElements; index++) {
    ...
    if (node) {
      base.append(Article1X10(node, undefined, onclickFunction, addFunction));
    } else {
      isFull = false;
      break;
    }
...
function displayKeywordTags(keywords, handleDeleteKeyword) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    document
      .getElementById('keyword-tags')
      .append(KeywordTag(id, keyword, handleDeleteKeyword));
  }
}
...
function displayKeywordArticles(keywordsObj, onclickFunction) {
  document
    .getElementById('keyword-articles')
    .append(getDisplayKeywordArticles(keywordsObj, onclickFunction));
}

export {
  getDisplayElement,
  displayKeywordTags,
  getDisplayKeywordArticles,
