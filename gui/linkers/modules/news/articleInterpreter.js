...
class ArticleHTMLInterpreter {
  constructor(
    minNumOccurences = 2,
    minTagsForAcceptance = 4,
    maxTagsForAcceptance = 25,
    decisionTree = getNewsDecisionTree(),
    acceptancePercentage = 0.6
  ) {
  ...
  interpret = (HTMLNode) => {
    if (!HTMLNode) return new InterpreterEnvironment();
      ...
      let interpreterEnvironment = new InterpreterEnvironment(
        this.minTagsForAcceptance,
        this.minNumOccurences,
        this.maxTagsForAcceptance,
        this.decisionTree,
        this.acceptancePercentage
      );
      HTMLNode.evaluate(
        interpreterEnvironment,
        interpreterEnvironment.tagCounter
      );
      console.log(interpreterEnvironment.articleClasses);
      interpreterEnvironment.checkArticleClasses();
      interpreterEnvironment.mergeArticleClasses();
  ...
  isNewsPage = (HTMLNode) =>
    this.interpret(HTMLNode).articleClasses.length > 0 ? true : false;

  isArticlePage = (HTMLNode) =>
    this.interpret(HTMLNode).articleClasses.length === 0 ? true : false;
}
