...
const idScores = {"itemprop": 15, "id": 5, "class": 2}

const hasNumberAtEnd = /-\d+$/

const unwantedTitleKeys = ["success"]
const unwantedImgKeys = ["thumb", "event", "icon", "nav", "logo", "measurement", "rating", "store", "logo", "seller", "header"]
const unwantedPriceKeys = ["list", 'questions', 'question', 'answer', 'dropdown', 'selector']
    ...
    for(var i=0; i<length; i++){
        ...
        if(id){
            ...
            for(let i=wantedKeywords.length; i>=0; i--){
                if(id.includes(wantedKeywords[i])) score += 10 *  idScore
            }

            if(unwantedTitleKeys.some(value => id.includes(value))) score /= 3
        }
...
function filterPriceList(priceList, titleID, getInfo=false, getOnlyFirstEquals=1, maxInterval=250){
    ...
    for(var i=0; i<length; i++){
        ...
        for(var j=0; j<idFieldsLength; j++){
            ...
            if(text){
                if(unwantedPriceKeys.some(item => text.includes(item))) break;
                if(get(infoSet.values, text, 0)<getOnlyFirstEquals) getTag = true;
        ...
        for(var j=0; j<idFieldsLength; j++){
            ...
            if(text){
                if(get(infoSet.values, text, 0)<getOnlyFirstEquals){
                    ...
                    if(field === "itemprop" || !unwantedImgKeys.some(item => lower.includes(item)))
                        getTag = true
                    else break
                }else{
                    getTag = false
    ...
    if(lenFilteredList>0){
        ...
        for(var i=0; i<lenFilteredList; i++){
            ...
            if(currency === mostFrequentCurrency){
                ...
                const lowerCaseTag = (tagProperties.id || tagProperties.class || "").toLowerCase();
                if(['delivery'].some(value => lowerCaseTag.includes(value))) continue;
...
function getScore(tagProperties, tagScoresDict, idDict = idScores, functionaultTagScore=0){
    let score = 0
    let keys = Object.keys(idDict)

    for(var i=keys.length; i>=0; i--){
        const key = keys[i];
        if(key in tagProperties) score += idDict[key];
    }
