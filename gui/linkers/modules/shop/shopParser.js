...
import { get_price_and_currency, check_after_price, check_for_price_interval, clean_up_price } from "../utils/priceFuncs.js"
...
class ShopHTMLParser{
                    ...
                    if(priceEnd>=lenText || check_after_price(text.slice(priceEnd))){
                        if(fullTag.length > tag.length){
                            if(!this.addedToPrices.last(-1)){
                                const lastTexts = this.texts.last(-1);

                                if(lastTexts.length>1) price[1] = clean_up_price(lastTexts);
    ...
    _handleText = (text, tag, handleTitle, handlePrice) => {
        ...
        if(lenText>0){
            if(this.possibleTitles>5) return handlePrice(text, lenText)
