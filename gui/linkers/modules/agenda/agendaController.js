import {
  retrieve_month_events,
  store_event,
  remove_event,
  update_value,
  retrieve_intersected_events,
} from './storageAgenda.js';
import {
  get_timestamp,
  days_of_month,
  monthShortName,
...
export default class AgendaController {
  ...
  getIntersectedEvents = async (month, year) => {
    return (
      await retrieve_intersected_events(
        this.database,
        moment([year, month]).unix(),
        userInfo.id
      )
    ).filter((obj) => obj.end_at);
  };
  ...
  addToEventArray = (agendaEvent, monthObj, day) => {
    ...
    if (eventArray) {
      ...
      let index = binary_search_for_insertion(
  ...
  addNewEvent = (agendaEvent) => {
    ...
    if (monthObj) {
      if (month === this.calendar.currentMonth)
        this.calendar.selectDay(date.date());
  ...
  addEvent = (
    ...
    startDate,
    monthObj,
    displayTimeFunction = undefined,
    monthEventsElement = undefined
  ) => {
    return agendaEvent.end_at
      ? this._addNewEventWithEndDate(agendaEvent, startDate, monthObj)
      : this._addEvent(
          startDate.date(),
          startDate.month(),
          startDate.year(),
  ...
  _addEvent = (
    ...
    let [position, moreThanOne] = this.addToEventArray(
      agendaEvent,
      monthObj,
      day
    );
    if (!moreThanOne) this.calendar.markAsBusy(year, month, day);
    monthEventsElement = monthEventsElement
      ? monthEventsElement
      : this.getElement(this.displayElements, year, month);
    return this.addToDisplay(
      position,
      this.getDayEvents(monthEventsElement, day),
      this.getEventCard(agendaEvent, displayTimeFunction)
    );
  };
  ...
  removeEvent = (agendaEvent) => {
    ...
    if (agendaEvent.children) this.removeEventChildren(agendaEvent, date);
      ...
      if (eventsArray.length === 0)
        this.handleNoEventArray(monthEventsObj, year, month, day, dayEvents);
    }
  };

  removeEventChildren = (agendaEvent, startDate) => {
    if (agendaEvent.children) {
      this.removeEventWithEndDate(
        agendaEvent,
        startDate,
        moment.unix(agendaEvent.end_at)
      );
      ...
      for (let i = children.length - 1; i >= 0; i--) {
        ...
        if (element.parentNode.children.length === 1)
          element.parentNode.parentNode.classList.add('no-events');
        element.remove();
      }
  ...
  removeEventWithEndDate = (agendaEvent, startDate, endDate) => {
    ...
    while (
      ...
      !startDate.isAfter(endDate, 'month')
    ) {
      month = startDate.month();
      year = startDate.year();
  ...
  _removeEventsFromMonth = (
    ...
    while (!startDate.isAfter(endDate, 'day') && startDate.month() === month) {
      ...
      eventsArray.splice(
        this.findEventPositionInArray(agendaEvent.id, eventsArray),
        1
      );

      if (eventsArray.length === 0)
        this.handleNoEventArray(monthEventsObj, year, month, startDate.date());

      startDate.add(1, 'day');
    }
  };

  handleNoEventArray = (monthEventsObj, year, month, day, dayElement) => {
    this.calendar.unmarkBusy(year, month, day);
    delete monthEventsObj[day];
    if (dayElement) dayElement.classList.add('no-events');
  };
  ...
  updateEventTitle = (agendaEvent) => {
    ...
    if (agendaEvent.children) {
      ...
      for (let i = 0; i < length; i++)
        children[i].children[2].innerText = agendaEvent.title;
    }
  };
  ...
  getMonthEvents = async (year, month) => {
    ...
    let monthObj = yearObj[month];
    if (!monthObj) {
      monthObj = await this.getMonthEventsFromDatabase(year, month);
  ...
  getMonthEventsFromDatabase = async (year, month) => {
    let endAt =
      month === 11
        ? get_timestamp(year + 1, 0)
        : get_timestamp(year, month + 1);
    return this.sortByDay(
      await retrieve_month_events(
        this.database,
        get_timestamp(year, month),
        endAt,
        userInfo.id
      )
    );
  ...
  createDisplayElement = async (year, month) => {
    let monthEventsObj = await this.getMonthEvents(year, month);
    let monthEventsElement = MonthEvents(
      year,
  ...
  setIntersectedEvents = async (
    month,
    year,
    monthEventsObj,
    monthEventsElement
  ) => {
    let intersectedEvents = await this.getIntersectedEvents(month, year);
    ...
    if (length > 0) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (startDate.year() !== currentYear) {
          ...
          currentMonthObj = await this.getMonthEvents(
            currentYear,
            currentMonth
          );
        ...
        let agendaEvent = this.getAgendaEventInArray(
          event,
          currentMonthObj[startDate.date()]
        );
        if (agendaEvent)
          this._addIntersectedEvent(
            agendaEvent,
            month,
            year,
            monthEventsObj,
  ...
  _displayDayEvents = (
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (event.end_at)
        this._displayDayEventWithEndDate(
          i,
          event,
          dayEventsElement,
          monthEventsObj,
  ...
  _displayDayEventWithEndDate = (
    ...
    if (endDate.isAfter(startDate, 'day')) {
      ...
      this.addToDisplay(position, dayElement, this.getEventCard(agendaEvent));
  };

  getEventCard = (agendaEvent, displayTimeFunction = undefined) =>
    EventCard(
      agendaEvent,
      agendaEvent.id,
      this.removeEvent,
  ...
  _addNewEventWithEndDate = (
    agendaEvent,
    startDate,
    monthElements = undefined
  ) => {
    let endDate = moment.unix(agendaEvent.end_at);
    ...
    if (startDate.isSame(endDate, 'day'))
      ...
      while (
        ...
        if (monthEventsObj) {
          monthEventElement = this.getElement(
            this.displayElements,
            year,
            month
          );

          if (monthEventElement)
            this._addToEventsAndDisplay(
              agendaEvent,
              monthEventsObj,
              monthEventElement,
              month,
      ...
      if (startDate.isSame(endDate, 'month')) {
        let children = agendaEvent.children;
        this._changeEventCardTime(
          children[children.length - 1],
          `Ends at ${endDate.format('HH:mm')}`
        );
      }
      ...
      if (monthElements) {
        ...
        this._changeEventCardTime(
          startElement,
          `Starts at ${startDate.format('HH:mm')}`
        );
        return startElement;
  ...
  _addEventWithEndDate = (
    ...
    this._changeEventCardTime(
      children[numChildren > 0 ? numChildren - 1 : 0],
      `Starts at ${startDate.format('HH:mm')}`
    );
    if (startDate.isSame(endDate, 'month')) {
      this._changeEventCardTime(
  ...
  updateDisplay = async (year, month, day = undefined) => {
    let yearObjs = this.displayElements[year];
    let monthObj;

    if (!yearObjs) {
      monthObj = await this.createDisplayElement(year, month);
